...
- **三语界面**：中文 / English / Deutsch（门户、登录、三端与培训题）
- **三层身份**：承运商组织 / 司机 / 车辆
...
- **危货联控核验（演示）**：勾选危货时走 `HazmatVerifyHub` mock（运输证/资格/目录/运单），通行码 `HAZ001` / `HAZBAD`
- **自动运营数据（无硬件）**：近 14 天多状态台账/看板；管理端可「重新灌入演示数据」

## 产品研发文档

- **[产品研发文档（总目录）](docs/产品研发文档.md)** — 文档地图、能力边界、实施路径摘要  
- [设计与开发文档](docs/设计与开发文档.md) · [系统流程图](docs/系统流程图.md) · [危货资质对接方案](docs/危货资质对接方案.md) · [人天评估](docs/产品研发实施人天评估.md) · [源码附录](docs/附录-完整源代码.md)
...
```
docs/     设计文档 + 系统流程图 + 源码附录
server/   Express + SQLite API、准入引擎、设备适配层
web/      React 三端（门户 / 司机 H5 / 门岗与后台）· 三语
scripts/  全流程自检
```
